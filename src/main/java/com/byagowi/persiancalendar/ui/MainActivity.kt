 ...
 * @author ebraminio
 */
open class MainActivity : DaggerAppCompatActivity(), SharedPreferences.OnSharedPreferenceChangeListener, NavigationView.OnNavigationItemSelectedListener, MActivity {
    override fun restartToSettingss() {
        restartToSettings();
    }

    override fun myself(): FragmentActivity {
        return this;
    }
    ...
    public fun onCreate1(savedInstanceState: Bundle?) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Utils.applyAppLanguage(this)
//        BrainActivity.getFirebaseId(this)
        //        getFirebaseId(this);
        NetGlobe.start(applicationContext)
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
            Blah.onSharedPreferenceChangeds(sharedPreferences,key,applicationContext,this)
    }
    companion object Blah {
        fun onSharedPreferenceChangeds(sharedPreferences: SharedPreferences?, key: String?,applicationcontext:Context,mactivity:MActivity) {

            var settingHasChanged = true
            ...
            if (key == PREF_SHOW_DEVICE_CALENDAR_EVENTS) {
                if (sharedPreferences?.getBoolean(PREF_SHOW_DEVICE_CALENDAR_EVENTS, true) == true) {
                    if (ActivityCompat.checkSelfPermission(mactivity.myself(), Manifest.permission.READ_CALENDAR) != PackageManager.PERMISSION_GRANTED) {
                        Utils.askForCalendarPermission(mactivity.myself())
                    }
                }
            }

            if (key == PREF_APP_LANGUAGE || key == PREF_THEME) {
                mactivity.restartToSettingss()
            }

            if (key == PREF_NOTIFY_DATE) {
                if (sharedPreferences?.getBoolean(PREF_NOTIFY_DATE, true) == false) {
                    mactivity.myself().stopService(Intent(mactivity.myself(), ApplicationService::class.java))
                    Utils.startEitherServiceOrWorker(applicationcontext)
                }
            }

            Utils.updateStoredPreference(mactivity.myself())
            UpdateUtils.update(applicationcontext, true)

            ViewModelProviders.of(mactivity.myself()).get(MainActivityModel::class.java).preferenceIsUpdated()
        }
    }
